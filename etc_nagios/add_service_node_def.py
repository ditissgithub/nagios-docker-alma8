...
def get_max_digit_count(start_node_no, last_node_no):
    ...
    return max(max_digit_count, 2)  # Ensure at least 3 digits
...
def main():
    ...
    node_type = get_user_input("Enter the Node Type to Add in Host Groups (e.g., master, mgmt, login): ")
    pv_net_address = get_user_input(f"Enter Private Network Address (Starting Pvt_IP Address of {node_type} node): ")
    prefix = get_user_input(f"Enter the Prefix Value for {node_type} node (e.g., master, mgmt, login): ")
